...
public class RoomIconPlacement {
	...
	class Room {
		...
		Room(int id) {
			...
			if (id==ROOM_KITCHEN) {start_x=5;start_y=5;}
			if (id==ROOM_BALLROOM) {start_x=261;start_y=69;}
			if (id==ROOM_CONSERVATORY) {start_x=548;start_y=5;}
			if (id==ROOM_BILLIARD) {start_x=581;start_y=293;}
			if (id==ROOM_LIBRARY) {start_x=579;start_y=452;}
			if (id==ROOM_STUDY) {start_x=550;start_y=675;}
			if (id==ROOM_HALL) {start_x=291;start_y=695;}
			if (id==ROOM_LOUNGE) {start_x=5;start_y=619;}
			if (id==ROOM_DINING) {start_x=5;start_y=327;}
