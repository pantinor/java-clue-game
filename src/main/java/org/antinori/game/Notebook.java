...
public class Notebook implements SerializableSFSType {
	...
	public boolean isLocationCardInHandOrToggled(Card card) {
		if (isCardInHand(card) || isCardToggled(card)) return true;
		return false;
	}
	
	public String toString() {
		String text = getPlayer().toString() +  "'s Notebook :";
		for (Entry entry : entries.values()) text += entry.toString();
		return text;
	}
	...
	public ArrayList<Card> canMakeAccusation() {
		...
		for (int i=0;i<NUM_SUSPECTS;i++) {
			...
			if (!isCardInHand(card) && !isCardToggled(card)) count ++;
		}
		
		if (count == 1) {
			for (int i=0;i<NUM_SUSPECTS;i++) {
				Card card = new Card(TYPE_SUSPECT, i);
				if (!isCardInHand(card) && !isCardToggled(card)) accusation.add(card);
			}
		}
		...
		if (accusation.size() != 3) accusation = null;
	...
	public Card randomlyPickCardOfType(int type) {
		...
		int total = 0;
		if (type==TYPE_SUSPECT) total = NUM_SUSPECTS;
		if (type==TYPE_ROOM) total = NUM_ROOMS;
		if (type==TYPE_WEAPON) total = NUM_WEAPONS;
		
		for (int i=0;i<total;i++) {
			Card card = new Card(type,i);
			if (isCardInHand(card) || isCardToggled(card)) continue;
			picks.add(card);
		}
